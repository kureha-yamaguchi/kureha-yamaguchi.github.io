---
layout: page
title: film
description: I've taken my film camera on a few trips now, here's some that I've taken that I'm particularly fond of.
img: assets/img/film/b8435e08-b338-46d6-912c-926ecdad6fa5.JPEG
importance: 5
images:
  photoswipe: true
---

<div class="pswp-gallery" id="gallery--craft">
  <div class="row row-cols-1 row-cols-md-3">
    <div class="col mb-3">
      <a href="{{ 'assets/img/film/IMG_3582.JPEG' | relative_url }}" data-pswp-width="3089" data-pswp-height="2048" target="_blank">
        {% include figure.liquid path="assets/img/film/IMG_3582.JPEG" class="img-fluid rounded z-depth-1" %}
      </a>
    </div>
    <div class="col-md-8 mb-3">
      <a href="{{ 'assets/img/film/IMG_3589.JPEG' | relative_url }}" data-pswp-width="3089" data-pswp-height="2048" target="_blank">
        {% include figure.liquid path="assets/img/film/IMG_3589.JPEG" class="img-fluid rounded z-depth-1" %}
      </a>
    </div>
    <div class="col mb-3">
      <a href="{{ 'assets/img/film/IMG_3584.JPEG' | relative_url }}" data-pswp-width="1668" data-pswp-height="2516" target="_blank">
        {% include figure.liquid path="assets/img/film/IMG_3584.JPEG" class="img-fluid rounded z-depth-1" %}
      </a>
    </div>
    <div class="col mb-3">
      <a href="{{ 'assets/img/film/IMG_1001.JPEG' | relative_url }}" data-pswp-width="871" data-pswp-height="1314" target="_blank">
        {% include figure.liquid path="assets/img/film/IMG_1001.JPEG" class="img-fluid rounded z-depth-1" %}
      </a>
    </div>
    <div class="col mb-3">
      <a href="{{ 'assets/img/film/IMG_0979.JPEG' | relative_url }}" data-pswp-width="1190" data-pswp-height="789" target="_blank">
        {% include figure.liquid path="assets/img/film/IMG_0979.JPEG" class="img-fluid rounded z-depth-1" %}
      </a>
    </div>
    <div class="col mb-3">
      <a href="{{ 'assets/img/film/IMG_1022.JPEG' | relative_url }}" data-pswp-width="1506" data-pswp-height="998" target="_blank">
        {% include figure.liquid path="assets/img/film/IMG_1022.JPEG" class="img-fluid rounded z-depth-1" %}
      </a>
    </div>
    <div class="col mb-3">
      <a href="{{ 'assets/img/film/IMG_0990.JPEG' | relative_url }}" data-pswp-width="1545" data-pswp-height="1024" target="_blank">
        {% include figure.liquid path="assets/img/film/IMG_0990.JPEG" class="img-fluid rounded z-depth-1" %}
      </a>
    </div>
    <div class="col mb-3">
      <a href="{{ 'assets/img/film/IMG_1023.JPEG' | relative_url }}" data-pswp-width="1545" data-pswp-height="1024" target="_blank">
        {% include figure.liquid path="assets/img/film/IMG_1023.JPEG" class="img-fluid rounded z-depth-1" %}
      </a>
    </div>
    <div class="col-md-8 mb-3">
      <a href="{{ 'assets/img/film/6499289f-31db-4fbb-abd0-2edf03b63f07.JPEG' | relative_url }}" data-pswp-width="1545" data-pswp-height="1024" target="_blank">
        {% include figure.liquid path="assets/img/film/6499289f-31db-4fbb-abd0-2edf03b63f07.JPEG" class="img-fluid rounded z-depth-1" %}
      </a>
    </div>
    <div class="col mb-3">
      <a href="{{ 'assets/img/film/b8435e08-b338-46d6-912c-926ecdad6fa5.JPEG' | relative_url }}" data-pswp-width="1545" data-pswp-height="1024" target="_blank">
        {% include figure.liquid path="assets/img/film/b8435e08-b338-46d6-912c-926ecdad6fa5.JPEG" class="img-fluid rounded z-depth-1" %}
      </a>
    </div>
    <div class="col mb-3">
      <a href="{{ 'assets/img/film/IMG_0985.JPEG' | relative_url }}" data-pswp-width="904" data-pswp-height="599" target="_blank">
        {% include figure.liquid path="assets/img/film/IMG_0985.JPEG" class="img-fluid rounded z-depth-1" %}
      </a>
    </div>
        </div>
        <div class="col mb-3">
      <a href="{{ 'assets/img/film/IMG_3550.JPEG' | relative_url }}" data-pswp-width="3089" data-pswp-height="2048" target="_blank">
        {% include figure.liquid path="assets/img/film/IMG_3550.JPEG" class="img-fluid rounded z-depth-1" %}
      </a>
    </div>
    <div class="col-md-8 mb-3">
      <a href="{{ 'assets/img/film/IMG_0222-1.JPEG' | relative_url }}" data-pswp-width="1940" data-pswp-height="1287" target="_blank">
        {% include figure.liquid path="assets/img/film/IMG_0222-1.JPEG" class="img-fluid rounded z-depth-1" %}
      </a>
    <div class="col mb-3">
      <a href="{{ 'assets/img/film/IMG_0525.JPEG' | relative_url }}" data-pswp-width="2279" data-pswp-height="1535" target="_blank">
        {% include figure.liquid path="assets/img/film/IMG_0525.JPEG" class="img-fluid rounded z-depth-1" %}
      </a>
    </div>
  </div>
</div>
